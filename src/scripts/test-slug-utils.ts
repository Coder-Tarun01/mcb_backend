import { buildJobSlug, extractIdFromSlug, toSlugSegment } from '../utils/slug';
...
async function run() {
  ...
  const seg = toSlugSegment('Senior Dévelopér, C++/C#');
  assert(seg === 'senior-developr-c-c', `toSlugSegment unexpected: ${seg}`);

  const slugWithLoc = buildJobSlug({
  ...
  const extracted1 = extractIdFromSlug('data-scientist-bengaluru-at-acme-xyz123');
  assert(extracted1 === 'xyz123', `extractIdFromSlug failed: ${extracted1}`);

  const extracted2 = extractIdFromSlug('job_987');
