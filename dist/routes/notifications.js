    ...
    try {
        ...
        await models_1.sequelize.transaction(async (transaction) => {
            for (const [index, row] of normalizedRows.entries()) {
                ...
                try {
                    const existingContactRows = await models_1.sequelize.query(`SELECT id, full_name, mobile_no, branch, experience
               ...
               LIMIT 1`, {
                   ...
                   WHERE id = :id`, {
                    ...
                    await models_1.sequelize.query(`INSERT INTO marketing_contacts (full_name, email, mobile_no, branch, experience, created_at)
...
router.get('/marketing/contacts', requireAdminKey, async (req, res) => {
    try {
        ...
        const contactRowsPromise = models_1.sequelize.query(`SELECT id, full_name, email, mobile_no, branch, experience, created_at
       FROM marketing_contacts
       ${whereClause}
...
router.post('/marketing/contacts', requireAdminKey, async (req, res) => {
    try {
        ...
        const [insertResult] = await models_1.sequelize.query(`INSERT INTO marketing_contacts (full_name, email, mobile_no, branch, experience, created_at)
       VALUES (:fullName, :email, :mobileNo, :branch, :experience, NOW())`, {
            replacements: {
                fullName,
                email,
        ...
        if (!contact) {
            const fallbackRows = await models_1.sequelize.query(`SELECT id, full_name, email, mobile_no, branch, experience, created_at
         FROM marketing_contacts
         WHERE email = :email
         ORDER BY id DESC
         LIMIT 1`, {
...
async function loadMarketingContactById(id) {
    ...
    const rows = await models_1.sequelize.query(`SELECT id, full_name, email, mobile_no, branch, experience, created_at
     FROM marketing_contacts
     WHERE id = :id
     LIMIT 1`, {
        replacements: { id },
